...
const unsigned int Config_Tbl_mode_a[2][CONF_REG_MAX] = {
        {0x00276400, 0x55440002, 0xA0000001, 0x00000002, 0x20000000, 0x00000004, 0x40000000},
        {0x00076c00, 0x21430002, 0xA0000001, 0x10000002, 0x10000000, 0x50000000, 0x40004000}
};
...
void gp22_start_toggle(int mode) {

    int ret;
    ret = ioctl(fd, GP22_START_PULSE, &mode);
    if (ret < 0) {
...
unsigned int gp22_rd_read_reg(unsigned char rd_reg) {
    ...
    int ret;
    ret = ioctl(fd, GP22_READ_REG, &dev_read);
    if (ret < 0) {
        LOGE("GP22 Read ERROR");
    }

    return dev_read.value;
}
...
float gp22_read_n_bytes(unsigned char read_addr, unsigned char fractional_bits) {
    ...
    Result = (float) (dev_read_byte.value / pow(2, fractional_bits));
...
//
    ...
    for (j = 0; j < 4; j++) {
        ...
        //                           2
        // ---------------------------------------------------
        result.calc_distance[j] = (float) (result.calc_time[j] / 6.671281904);
//                                  * (float) cableType;    //distance [m]	// 2000000 kmps = 6.671281904 speed of light in vacuum

        //Distance_Result[j] = Distance_Result[j] - (Distance_Result[j] * 0.2);

        LOGW("|  %d  |    %.3f | %5d.%03d  | %5d.%03d  | \n", j + 1, result.average_value[j],
...
void gp22_measure_start(void) {
    ...
    gp22_start_toggle(gp22.measuremode);
...
//            LOGD(" %02d. Measure RAW Value = %.3f \n", i, result.read_value[h]);    // RAW value
    ...
    LOGD("[GP22_1101] result.calc_distance[0] = %.3f\n", result.calc_distance[0]);
    LOGD("[GP22_1101] result.calc_distance[1] = %.3f\n", result.calc_distance[1]);
...
//    bnc_length = result.calc_distance[5] * (float) cableType;
    bnc_length = (result.calc_distance[5] + result.calc_distance[4]) * (float) cableType;
...
JNIEXPORT void JNICALL Java_com_sscctv_seeeyes_SpiPort_GpioOn(JNIEnv *env, jobject obj) {
    char high = 1;
    ioctl(fd, GP22_BNC_PIN_SET, &high);
}

JNIEXPORT void JNICALL Java_com_sscctv_seeeyes_SpiPort_GpioOff(JNIEnv *env, jobject obj) {
    char low = 0;
...
Java_com_sscctv_seeeyes_SpiPort_setCableType(JNIEnv *env, jobject obj, double i) {

    cableType = i;
    LOGD("Cable Type ===== %f", cableType);

}

JNIEXPORT double JNICALL
Java_com_sscctv_seeeyes_SpiPort_getCableType(JNIEnv *env, jobject obj) {
    return cableType;
}
